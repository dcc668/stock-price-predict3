...
def seq2seq(feature_len=1, after_day=1, input_shape=(20, 1)):
    ...
    densor_output = Dense(
        units=feature_len, activation='linear', name='output')
...
def seq2seq_model(feature_len, after_day, input_shape,batch_size,epochs,_class,scaler,
    ...
    # checkoutpoint
    checkpointer = ModelCheckpoint(filepath="model/model-3/weights.h5",
    ...
    train_score = model.evaluate(
        x=x_train, y=y_train, batch_size=batch_size, verbose=0)
    print('Train Score: %.8f MSE (%.8f RMSE)' %
          (train_score[0], math.sqrt(train_score[0])))

    validate_score = model.evaluate(
        x=x_validate, y=y_validate, batch_size=batch_size, verbose=0)
...
if __name__ == '__main__':
    ...
    for index in range(len(class_list)):
        ...
        plot_predict(after_day,y_validate, validate_predict, test_predict,file_name=file_name)
